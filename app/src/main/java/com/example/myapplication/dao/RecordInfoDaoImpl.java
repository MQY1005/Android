...
import com.example.myapplication.utils.DBHelper2;

import java.util.ArrayList;
import java.util.List;


public abstract class RecordInfoDaoImpl implements RecordInfoDao{
    private DBHelper2 helper;
    private SQLiteDatabase db;

    public RecordInfoDaoImpl(Context context){
        helper = DBHelper2.getInstance(context);
    }
    ...
    public List<Records> select(String username) {
        ...
        Cursor cursor = db.query(DBHelper2.TBL_NAME_RECORD,null,"user_name=?",
...
//
//        db = helper.getWritableDatabase();
//        db.update(DBHelper2.TBL_NAME_RECORD,values,"user_name = ?",new String[]{records.getUsername()});
//        db.close();
//    }
//
//    @Override
